...
class Blockchain {
    ...
    generateGenesisBlock(){
        // Add your code here
        let self = this;
        let newBlock = new Block.Block("Genesis Block - First block in the chain for UDACITY Project 2 by MJFullStack - Genesis block");
    ...
    getBlock(height) {
        ...
        return new Promise(function(resolve, reject) {
            self.bd.getLevelDBData(height).then((gotBlock) => {
                // console.log(`getBlock: Retrieved Block ${height}. Contents are ...\n`, gotBlock);
                resolve(gotBlock);
            })
            .catch( (err) => {
                if (err) {
